/* w3codecolor ver 1.31 by w3schools.com , Actually from w3schools.com*/
function Highlight() {
...
"function","if","in","new","return","switch","throw","try","typeof","while","with","Manager",
"__managerProto__","getScript","$"];
if (!document.getElementsByClassName) {return;}
...
for (i = 0; i < x.length; i++) {
  ...
  if (ele) {
    ...
    if (ele =="true" || ele == "false" || ele == "null" || isNaN(ele) == false || ele == "Manager") {  
  ...
  text = text.replace(/Manager/imgs,"<span style='color:#FF9E59'>Manager</span>");
  text = text.replace(/\$/imgs,"<span style='color:#FF9E59'>$</span>");

  text += c;
}
z.innerHTML = text;
if($(z).hasClass('jsHigh')){
  var $button = $("<button />",{
    "class": "btn tryit size-0",
  }).html("Try This at editor"),
  $button2 = $("<button />",{
    "class": "btn tryit size-0",
  }).html("Demo"),
  $btnGroup = $("<div />",{
  ...
  $button.click(function(){
    var lsm = new Manager.LSM('MngrJsDocs');
    lsm.set('EditorContent',$text);
    Manager.navigateTo("/editor");
  });
